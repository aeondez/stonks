...
const safeSet = async (key, value, token = null) => {
  try {
...
function PlayerSessionTab({ debt, crew, rollConfig, stocks, houseRules }) {
  ...
  const [open, setOpen] = useState({ checklist:false, debt:false, contractors:false, payout:false, medical:false, shore:false, training:false, repairs:false, houserules:false });
...
function WardenView({ stocks, setStocks, headlines, setHeadlines, history, setHistory, date, setDate,
  ...
  // Authenticated write — includes session token for server-side validation
  ...
  if (view === "warden") {
    return (
      <WardenView
        ...
        storedPin={sessionToken} setStoredPin={setSessionToken}
