...
- **Codex tab** — a fourth player tab (alongside Market, Jobs, Downtime) for campaign lore: locations, NPCs, factions, or anything else worth documenting. Entries are grouped by a freeform category tag and expand in place to show the full text. Warden → Codex is the matching editor: add/edit/delete entries, tag them with a category, and flip each between HIDDEN and VISIBLE independently — new entries start hidden so you can stage content ahead of time and reveal it when the players actually find it. Hidden entries are hidden in the UI the same way locked Catalog perks already are (not filtered server-side).
- **Cloud backups** — every successful Warden login now automatically snapshots the room's full state (stocks, headlines, history, jobs, crew, debt, portfolio, catalogs, black market, house rules, codex, and settings) to Redis, keeping the last 3. A "SNAPSHOT NOW" button in Warden → Settings takes one on demand. Each snapshot can be restored from Settings with a confirmation prompt, reusing the same restore path as the existing local-file import.
...
- **Export Backup was missing house rules and roll config** — the manual EXPORT BACKUP bundle never included `houseRules` or `rollConfig` (a restored backup silently fell back to default roll config every time), despite both being live, persisted data. Now included, along with the new `codex` data.
- **Security: session tokens never actually validated** — `verifySession` in `api/store.js` compared the stored session flag with `val === "1"`, but the Upstash client auto-deserializes the string `"1"` back into the number `1` on read, so the comparison silently failed for every token, always. Every Warden write (including the Shore Leave settings above) was failing 403 in the background — invisible because the UI updates optimistically and `safeSet` never checks the response status — and every failed write counted toward the 5-attempt lockout, which is what was actually causing rooms to self-lock. Fixed by coercing to `String(val) === "1"`.
- **Room PIN could silently expire mid-campaign** — the PIN's Redis key has the same 90-day expiration as everything else in a room, but unlike everything else, nothing ever refreshed it during normal play (every other key's TTL gets bumped by the `_active` write-tracking; the PIN is only ever written once, at creation or an explicit change). A campaign that never changes its PIN could hit 90 days of elapsed time and lock the Warden out entirely, with no in-app recovery path once it happens. The PIN's TTL is now refreshed on every successful login and on every authenticated write, matching how the rest of the room's data already behaves.
- **Codex category dropdown was white-on-white** and **category/entry lists weren't alphabetized** — the `<select>` inherited the app's light text color into the browser's native (light-mode) popup background; added `colorScheme: "dark"`. Category derivation and entry lists (Player tab and Warden panel) now sort case-insensitively by name/title instead of leaving entries in raw insertion order.
- **Codex title/body input stole focus after one keystroke** — the category field was defined as an inline component inside `CodexPanel`'s render body, so React remounted it (re-triggering its `autoFocus`) on every keystroke in any field. Moved to a proper top-level component.
